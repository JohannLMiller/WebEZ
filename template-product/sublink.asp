...
%>		
<!--title-table-->		
	<TABLE width=100% ALIGN=center BORDER=0 bordercolor=pink  CELLSPACING=1 CELLPADDING=1>
	<TR>
   ...
   <%
    set rs=server.CreateObject("adodb.recordset")
				...
				if err.number <> 0 then
                		...
                		<A HREF="main2.asp?cat=<%=cat%>&amp;catNo=<%=rs("catNo")%>&amp;subcat=<%=rs("subcat")%>&amp;subcatNo=<%=rs("subcatNo")%>" class="sub" ><%=rs("subcat")%></A>
